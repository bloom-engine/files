...
const getInputData = () => {
  ...
  document
    .querySelectorAll(
      "#bloom-container input[data-label], #bloom-container select[data-label]"
    )
    .forEach((input) => {
...
const getOutputLabels = () => {
  return Array.from(
    document.querySelectorAll(
      "#bloom-container [data-label]:not(input):not(select)"
    )
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  bloomContainer.querySelector("form").addEventListener("submit", (event) => {
    event.preventDefault();

    bloomContainer.style.position = "relative";

    const fragment = document.createRange().createContextualFragment(`
            <div class="bloom-calculator-mask">
            <div class="bloom-loader-container">
              <div class="bloom-loader"></div>
              <div class="bloom-loading-text">Calculating Results</div>
            </div>
          </div>  
        `);

    bloomContainer.prepend(fragment);
    ...
    fetch(`${baseUrl}/v3.0`, {
        ...
        Object.keys(output).forEach((result) => {
          ...
          try {
            switch (checkType(dataVal)) {
              case "SV":
                document.querySelector(`[data-label="${result}"]`).innerText =
                  dataVal;
                break;
              case "1D":
              ...
              case "2D":
                dataVal.forEach((ele, index) => {
                  ...
                  ele.forEach((e, i) => {
                    const cell = clonedNode.querySelector(
                      `[data-col="${i + 1}"]`
                    );
                    if (cell) {
      ...
      .finally(() => {
        bloomContainer.querySelector(".bloom-calculator-mask")?.remove();
      });
  });
});
